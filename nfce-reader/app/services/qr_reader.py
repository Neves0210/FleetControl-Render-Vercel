...
def _to_uint8(image: np.ndarray) -> np.ndarray:
    """Garante que a imagem está em uint8 (necessário para PIL e pyzbar)."""
...
def read_with_opencv(image: np.ndarray) -> str | None:
    ...
    try:
        data, _, _ = detector.detectAndDecode(_to_uint8(image))
...
def read_with_zxing(image: np.ndarray) -> str | None:
    try:
        ...
        if len(image.shape) == 2:
            # Escala de cinza — zxingcpp aceita diretamente como numpy array
